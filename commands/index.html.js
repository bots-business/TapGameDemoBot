        ...
        <div v-if="activePage === 'Mine'">
          ...
          <div v-for="(items, subPageName) in buildings" :key="subPageName">
            <div v-if="mineSubPageName === subPageName">
              <div v-for="(building, key) in items" :key="key" class="event-card">
                <div class="event-header">
                  <img :src="building.icon" :alt="building.title" class="event-icon">
                  <div class="event-details">
                    <h3>{{ building.title }}</h3>
                    <p>Hour income</p>
                    <span class="event-profit icon-coin">{{ building.l1.income }}</span>
                  </div>
                </div>
                <div class="level-info d-flex justify-content-between align-items-center">
                  <div class="level">lvl {{0}}</div>
                  <div class="total-profit icon-coin" style="margin-right: 40px;">{{ building.l1.cost }}</div>
                </div>
              </div>
            </div>
          </div>
      ...
      const app = Vue.createApp({
        ...
        methods: {
          ...
          trackTapWork() {
            ...
            // see: bot command syncBalance
            fetch(this.ApiUrls.trackTapWork, {
              method: 'POST',
              headers: {
                'Content-Type': 'application/json'
              },
              body: JSON.stringify({ balance: this.user.balance, energy: this.user.energy })
            }).then(() => {
              console.log('Track tap work posted');
            });
          }
        },
