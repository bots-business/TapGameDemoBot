...
describe('Building Upgrades. Please wait 10 secs...', function() {
  ...
  function getUpgradedBuilding(balance) {
    // try to upgrade "Upgrade mine" - it is the first building
    let building = document.querySelector('.event-card');
    if (!building) {
  ...
  function clickUpgradeButton(){
    // h2 test must be "Upgrade mine"
    let modalTitle = document.querySelector('#TopModal h2');
    if (!modalTitle || modalTitle.textContent !== 'Upgrade mine') {
      throw new Error('Upgrade modal not shown');
    }
    let upgradeButton = document.querySelector('#TopModal .btn');
    if (!upgradeButton) {
    ...
    if(upgradeButton.disabled) {
      throw new Error('Upgrade button is disabled');
    }
    upgradeButton.click();
  }

  function verifyUpgrading(done) {
    // cur balance must be 100
    let balance = getCurrentBalance();
    if (balance < 100) {
      throw new Error('Balance is not enough for upgrade');
    }

    let building = getUpgradedBuilding()

    building.click();
    setTimeout(function() {
      clickUpgradeButton();

      // modal must be closed automatically after upgrade and 200 ms
      setTimeout(function() {
        ...
        // check balance
        let newBalance = getCurrentBalance();
        if (newBalance !== balance - 100) {
          return done(new Error('Balance is not decreased after upgrade. Current balance: ' + newBalance));
        }
        done();
      }, 5000);
  ...
  it('Can upgrade a building.', function(done) {
    this.timeout(15000);  // 15 seconds for this test

    openSubPage('Work', function() {
      // tap work for 200 coins
      clickTapButton(200, done);

      // wait for tap work to complete
      setTimeout(function() {
        openSubPage('Mine', function() {
          verifyUpgrading(done);
        });
      }, 5000);
    });
  });
  ...
  after(function(done) {
    this.timeout(15000);
    closeUpgradeModalAfterTest(done);
  })
});
...
function closeUpgradeModal(done, callback) {
  ...
  if (modalInstance) {
    modalInstance.hide();
    // remove all backdrops - we have conflict with multiple modals (Mocha also uses modal)
...
function closeUpgradeModalAfterTest(done) {
  // check if modal exist
  let modal = document.querySelector('#TopModal');
  if (modal && modal.classList.contains('show')) {
    return closeUpgradeModal(done);
  }
  done();
}
